...
class RSingleTickerView: UIView
{
    ...
    private var mForceAnimate:Bool = true; // true: animating for (text width < view width )
    ...
    func startWithDelay(DelaySeconds delay:Float)
    {
        if mTimerDelay == nil && mTimer == nil
        {
            mTimerDelay = NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: #selector(RSingleTickerView.start), userInfo: nil, repeats: false)
        }
    }
    ...
    func start()
    {
        ...
        if mTimer == nil
        {
            mTimer = NSTimer.scheduledTimerWithTimeInterval(NSTimeInterval(mMoveInterval), target: self, selector: #selector(RSingleTickerView.tick(_:)), userInfo: nil, repeats: true)
        }
    }
